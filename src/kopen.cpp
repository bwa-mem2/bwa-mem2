...
#define __err_connect(func) do { perror(func); freeaddrinfo(res); return -1; } while (0)
	...
	if (setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on)) == -1) {close(fd); __err_connect("setsockopt"); }
	if (setsockopt(fd, SOL_SOCKET, SO_LINGER, &lng, sizeof(lng)) == -1) {close(fd); __err_connect("setsockopt");}
	if (connect(fd, res->ai_addr, res->ai_addrlen) != 0) {close(fd); __err_connect("connect");}
	freeaddrinfo(res);
	return fd;
#undef __err_connect
}
...
static int http_open(const char *fn)
{
	...
	if (proxy == 0) {
		...
		for (q = host; *q && *q != ':'; ++q);
		if (*q == ':') *q++ = 0; 
        const char *port_ = *q ? q : "80";
		port = strdup(port_);
		path = strdup(fn);
	}
	int port_int = -1;
	sscanf(port, "%d", &port_int); assert(port_int >= 0 && port_int <= 65535);
...
static int ftp_open(const char *fn)
{
	...
	sprintf(host2, "%d.%d.%d.%d", pasv_ip[0], pasv_ip[1], pasv_ip[2], pasv_ip[3]); // no size check needed, as host2 size is weel beyond the upper bound of string being formatted.
	sprintf(port2, "%d", pasv_port);   // same as above
		...
		if (*p == '<') { // pipe open
			...
			if (pid == 0) { /* the child process */
				...
				else
				{
					...
					const char *ar[5], s1[] = "/bin/sh", s2[] ="sh", s3[]="-c";
					ar[0] = s1, ar[1] = s2, ar[2] = s3, ar[3] = p+1, ar[4]=NULL;
					execve(ar[0], (char **)  &ar[1], NULL);
				}
				exit(1);
			} else { /* parent process */
				close(pfd[1]);
